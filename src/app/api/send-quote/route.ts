...
export async function POST(request: NextRequest) {
  try {
    ...
    const { formData, calculations, pdfBuffer, pdfFilename, refNumber } = body;
    ...
    // Convertir PDF de base64 a buffer para adjunto
    ...
    const results = await EmailService.sendMultiple([
      () => EmailService.sendQuoteToClient(client, refNumber, clientTemplate, pdfAttachment),
      () => EmailService.sendQuoteInternal(client, refNumber, internalTemplate, pdfAttachment)
    ]);
